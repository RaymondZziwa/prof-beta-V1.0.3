...
const BuwamaChickenFeedsStockTaking = () => {
    const [isStockDataLoading, setIsStockDataLoading] = useState(true)
    const [feedsStock, setFeedsStock] = useState([])
    ...
    useEffect(() => {
        const fetchData = async () => {
          if (isStockDataLoading) {
            const res = await axios.post('http://82.180.136.230:3005/buwamafetchchickenfeedstock', {
              token: localStorage.getItem('token')
            });
            console.log(res.data);
            if (Array.isArray(res.data)) {
              setIsStockDataLoading(false);
              setFeedsStock(res.data);
            }
          }
        };
    ...
    return(
        <Row>
            ...
            <div className="col align-self-center" style={{marginTop:'60px'}}>
                <h1 style={{textAlign:'center'}}>Buwama Chicken Feeds Stock Taking</h1>
                <Row>
...
export default BuwamaChickenFeedsStockTaking
...
const BuwamaGeneralStoreOutgoingForm = () => {
    ...
    const fetchItems = async () => {
        const res = await axios.post('http://82.180.136.230:3005/buwamafetchallitems', {
    ...
    const saveRestockData = async event => {
        event.preventDefault()
         let res = await axios.post('http://82.180.136.230:3005/buwamasavegeneralstorerestockdata',{
            token:localStorage.getItem('token'),
             category: category,
    ...
    return(
        ...
        <div className="col align-self-center" style={{marginTop:'60px'}}>
            <h1 style={{textAlign:'center'}}>Buwama General Store Outgoing Form</h1>
            ...
            <select class="form-select" aria-label="Default select example" style={{ height: "60px", color: "#8CA6FE;" }} onChange={sourceInput} required>
                ...
                <div className="mb-3">
                <textarea type="text" className="form-control" rows="6" id="floatingInput" placeholder="Destination Information" style={{ color: "#8CA6FE", height: '130px', width: '300px' , marginTop:'10px'}} onChange={externalSourceInfoInput} />
                </div>
            <div className="form-floating mb-3">
...
export default BuwamaGeneralStoreOutgoingForm
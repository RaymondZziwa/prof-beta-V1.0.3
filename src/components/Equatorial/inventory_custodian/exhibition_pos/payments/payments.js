...
const PaymentModule = ({ servicesList, items, setItems, total }) => {
    ...
    useEffect(() => {
        const fetchExhibitionList = async () => {
            let res = await axios.post('http://82.180.136.230:3005/fetchpreexhibitionlist', {
    ...
    const handleSubmit = async (event) => {
        event.preventDefault()
       let res = await axios.post('http://82.180.136.230:3005/exhibitioncheckout',{
        token: localStorage.getItem('token'),
    ...
    return(
        <>  
            ...
            <select class="form-select" aria-label="Default select example" style={{ height: "60px", color: "#8CA6FE;" }} onChange={(e)=>setExhibitionId(e.target.value)} required>
                        <option selected>Select Exhibition Name</option>

...
const EquatorialManageExternalReceipts = () => {
    ...
    const markReceiptAsDelivered = async event => {
        event.preventDefault()
        if(receiptData){
            let res = await axios.post('http://82.180.136.230:3005/equatorialmarkreceiptasdelivered',{
            token: localStorage.getItem('token'),
            items: receiptItems,
            receiptNumber: receiptNumber
            })
...
export default EquatorialManageExternalReceipts
...
const EquatorialShopStockTaking = () => {
  ...
  useEffect(() => {
      const fetchData = async () => {
        if (isStockDataLoading) {
          const res = await axios.post('http://82.180.136.230:3005/fetchequatorialshopstock', {
    ...
    return(
      ...
      <Row className='row justify-content-center' style={{marginTop:'60px'}}>
          <Col className='col-8'>
              <h1 style={{textAlign:'center'}}>Equatorial Shop Department Stock Taking</h1>
...
export default EquatorialShopStockTaking